...
import {CURRICULUM_CATALOG_DOCUMENT_ID} from './schemaTypes/documents/curriculumCatalog'
import {CURRICULUM_RELEASE_POINTER_DOCUMENT_ID} from './schemaTypes/documents/curriculumReleasePointer'
    ...
    .items([
      S.listItem()
        ...
        .child(
          S.list()
            .title('Content reports')
            .items([
              S.listItem().title('New').icon(FeedbackIcon).child(reportsByStatus('New reports', 'new')),
              S.listItem()
                .title('In progress')
                .icon(FeedbackIcon)
                .child(reportsByStatus('Reports in progress', 'inProgress')),
              S.listItem().title('Resolved').icon(FeedbackIcon).child(reportsByStatus('Resolved reports', 'resolved')),
              S.listItem()
                .title('Won’t fix')
                .icon(FeedbackIcon)
      ...
      S.listItem()
        ...
        .child(
          S.list()
            ...
            .items([
              S.listItem()
                .title('Catalog & module order')
                .icon(BookIcon)
                .child(S.document().schemaType('curriculumCatalog').documentId(CURRICULUM_CATALOG_DOCUMENT_ID)),
              S.divider(),
      ...
      S.listItem()
        ...
        .child(
          S.list()
            .title('Assessment')
            .items([S.documentTypeListItem('question').title('Questions').icon(HelpCircleIcon)]),
        ),
      S.listItem()
      ...
      S.listItem()
        ...
        .icon(RocketIcon)
        .child(S.document().schemaType('curriculumReleasePointer').documentId(CURRICULUM_RELEASE_POINTER_DOCUMENT_ID)),
