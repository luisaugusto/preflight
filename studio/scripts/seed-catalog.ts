...
function draftTimestamp(contentVersion: string): string {
  ...
  if (!match)
    throw new Error(`Draft content version must start with YYYY.MM.DD: ${contentVersion}`);
...
function canonicalAcsCode(input: string) {
  const match = input.trim().match(/^([A-Z]{2})\.([IVXLCDM]+)\.([A-Z]+)\.([KRS])(\d+)([a-z]?)$/i);
...
function figureStableId(moduleId: string, filename: string): string {
  return `${normalizeId(moduleId)}-${normalizeId(path.basename(filename, path.extname(filename)))}`;
}
...
async function listFigureFiles(moduleId: string): Promise<string[]> {
  const entries = await readdir(figureAssetsPath(moduleId), { withFileTypes: true });
  return entries
    .filter(
...
function multipleChoiceSpec(options: string[], correctIndex: number, keyPrefix: string) {
...
function questionDocument(module: ModuleContent, record: QuestionRecord): SanitySeedDocument {
  ...
  if (question.type === 'multipleChoice') {
    ...
    common.imageAnswer = {
      ...
      stimulusFigure: reference(
        'figure',
        figureStableId(module.id, basenameFromUri(question.image.uri)),
      ),
    ...
    content: [
      ...
      lessonBlock('workedExample', 'Worked example', lesson.workedExample, 2),
      {
        ...lessonBlock(
          ...
          'Apply this concept before continuing.',
          3,
        ),
...
function buildDocuments(
  ...
  module.glossary.forEach((term) => {
    glossaryBySection.set(term.sectionId, [
      ...(glossaryBySection.get(term.sectionId) ?? []),
      term.id,
    ]);
  });
  ...
  const acsDocuments = allAcsCodes.map((inputCode) => {
    ...
    return {
      ...
      description: `Preflight curriculum content aligned to ${parsed.code}.`,
  ...
  const figureDocuments = figureFiles.map((filename) => {
    ...
    const fallbackSection =
      sortedSections.find((section) => section.order === Number(chapterMatch?.[1])) ??
      sortedSections[0];
    ...
    return {
      ...
      tags: [module.id, 'representative-figure'],
    };
  });
  ...
  const sectionDocuments = sortedSections.map((section) => {
    ...
    const sectionFigure = figureFiles.find(
      (filename) => figureContexts.get(filename)?.section.id === section.id,
    );
    ...
    return {
      ...
      ...(sectionFigure
        ? { heroFigure: reference('figure', figureStableId(module.id, sectionFigure)) }
        : {}),
      lessons: refs(
        'lesson',
      ...
      quizQuestions: refs(
        'question',
        section.quiz.map((question) => question.id),
      ),
  ...
  const moduleDocument: SanitySeedDocument = {
    ...
    ...(representativeFigure
      ? { coverFigure: reference('figure', figureStableId(module.id, representativeFigure)) }
      : {}),
    sections: refs(
      'section',
      sortedSections.map((section) => section.id),
    ),
    finalExamQuestions: refs(
      'question',
      module.exam.map((question) => question.id),
    ),
    glossaryTerms: refs(
      'glossaryTerm',
      module.glossary.map((term) => term.id),
    ),
...
function releaseDocument(catalog: CurriculumCatalog, options: BuildOptions): SanitySeedDocument {
  const releaseStableId = `content-${normalizeId(catalog.contentVersion)}`;
  return {
    ...
    releaseNotes: portableText(
      `Generated four-module FAA curriculum covering ${catalog.modules.map((module) => module.shortTitle).join(', ')}.`,
...
function prepareSeedDocuments(documents: SanitySeedDocument[]): SanitySeedDocument[] {
  return documents.map((document) => {
    const prepared = weakenContentReferences({
      ...document,
      _id: publish ? document._id : `drafts.${document._id}`,
    }) as SanitySeedDocument;
    if (publish) return prepared;
...
function weakenContentReferences(value: unknown): unknown {
...
function validateDocuments(documents: SanitySeedDocument[]) {
  ...
  for (const document of documents) {
    ...
    if (publish === document._id.startsWith('drafts.')) {
      throw new Error(
        `Unexpected document ID for ${publish ? 'publish' : 'draft'} mode: ${document._id}`,
      );
    }
  }
}
...
async function withRetries<T>(label: string, operation: () => Promise<T>): Promise<T> {
  ...
  for (let attempt = 0; attempt <= delays.length; attempt += 1) {
    try {
      ...
      console.warn(`${label} failed; retrying in ${delays[attempt]}ms (attempt ${attempt + 2}).`);
...
async function uploadImages(
  ...
  for (const filename of figureFiles) {
    ...
    const asset = (await withRetries(`Image upload ${moduleId}/${filename}`, () =>
      client.assets.upload(
        'image',
        createReadStream(path.join(figureAssetsPath(moduleId), filename)),
...
async function uploadJsonFile(
  ...
  return (await withRetries(`JSON upload ${filename}`, () =>
...
async function writeDocuments(
  ...
  for (let offset = 0; offset < documents.length; offset += chunkSize) {
    ...
    await withRetries(`Document batch ${offset + 1}–${offset + chunk.length}`, () =>
      transaction.commit({ visibility: 'sync' }),
    );
    console.log(
      `${publish ? 'Published' : 'Seeded draft'} documents ${offset + 1}–${offset + chunk.length} of ${documents.length}`,
    );
  }
}

function uniqueDocuments(documents: SanitySeedDocument[]): SanitySeedDocument[] {
  const byId = new Map<string, SanitySeedDocument>();
  for (const document of documents) {
    const existing = byId.get(document._id);
    if (existing && canonicalJson(existing) !== canonicalJson(document)) {
      throw new Error(`Conflicting generated Sanity documents share ID ${document._id}.`);
    }
...
async function buildCatalogDocuments(
  ...
  options: Omit<BuildOptions, 'imageAssetIds'>,
  imageAssetsByModule: Map<string, Map<string, string>>,
) {
  const documents: SanitySeedDocument[] = [];
  for (const module of catalog.modules) {
    const figureFiles = await listFigureFiles(module.id);
    if (!figureFiles.length) {
      throw new Error(`No representative figures found in ${figureAssetsPath(module.id)}.`);
    }
    ...
    for (const { question } of figureContexts.values()) {
      if (question && !figureFiles.includes(basenameFromUri(question.image.uri))) {
        throw new Error(
          `Image question ${question.id} references a missing asset: ${question.image.uri}`,
        );
      }
    }
    const imageAssetIds = imageAssetsByModule.get(module.id);
    if (!imageAssetIds) throw new Error(`Missing image asset map for module ${module.id}.`);
    documents.push(
      ...buildDocuments(module, figureFiles, questionRecords, figureContexts, {
        ...options,
        imageAssetIds,
      }),
    );
  }
  documents.push(releaseDocument(catalog, { ...options, imageAssetIds: new Map() }));
  return uniqueDocuments(documents);
}

async function main() {
  const catalog = await loadCanonicalCatalog();
  const bundleRaw = canonicalJson(catalog);
  const bundleChecksum = sha256(bundleRaw);
  const placeholderAssetsByModule = new Map<string, Map<string, string>>();
  for (const module of catalog.modules) {
    const figureFiles = await listFigureFiles(module.id);
    placeholderAssetsByModule.set(
      module.id,
      new Map(
        figureFiles.map((filename) => [
          filename,
          `image-placeholder-${sha256(`${module.id}/${filename}`).slice(0, 40)}-1x1-jpg`,
        ]),
      ),
    );
  }
  const now = new Date().toISOString();
  const defaultContentTimestamp = publish ? now : draftTimestamp(catalog.contentVersion);
  const dryRunDocuments = prepareSeedDocuments(
    await buildCatalogDocuments(
      catalog,
      {
        bundleUrl: `https://example.invalid/preflight/${catalog.contentVersion}/catalog.json`,
        bundleChecksum,
        manifestUrl: `https://example.invalid/preflight/${catalog.contentVersion}/manifest.json`,
        publishedAt: defaultContentTimestamp,
      },
      placeholderAssetsByModule,
    ),
  );
      ...
      JSON.stringify(
        {
          ...
          source: 'src/content/catalog.json',
          moduleIds: catalog.modules.map((module) => module.id),
          contentVersion: catalog.contentVersion,
          figures: [...placeholderAssetsByModule.values()].reduce(
  ...
  const releaseDocumentId = documentId(
    'contentRelease',
    `content-${normalizeId(catalog.contentVersion)}`,
  );
  const existing = await client.fetch<Array<{ _id: string; publishedAt?: string }>>(
    `*[_id in $ids]{_id, publishedAt}`,
    {
      ids: [
        ...catalog.modules.map((module) => documentId('module', module.id)),
        releaseDocumentId,
      ],
    },
  );
  const publishedAt =
    existing.find((document) => document._id === releaseDocumentId)?.publishedAt ??
    defaultContentTimestamp;

  const imageAssetsByModule = new Map<string, Map<string, string>>();
  for (const module of catalog.modules) {
    imageAssetsByModule.set(
      module.id,
      await uploadImages(client, module.id, await listFigureFiles(module.id)),
    );
  }
  const bundleAsset = await uploadJsonFile(
    client,
    `catalog-${catalog.contentVersion}.json`,
    bundleRaw,
  );
  if (!bundleAsset.url)
    throw new Error('Sanity did not return a CDN URL for the content bundle asset.');
  ...
  const manifestAsset = await uploadJsonFile(
    client,
    `catalog-${catalog.contentVersion}-manifest.json`,
    manifestRaw,
  );
  if (!manifestAsset.url)
    throw new Error('Sanity did not return a CDN URL for the content manifest asset.');

  const documents = prepareSeedDocuments(
    await buildCatalogDocuments(
      catalog,
      {
        bundleUrl: bundleAsset.url,
    ...
    JSON.stringify(
      {
        ...
        moduleIds: catalog.modules.map((module) => module.id),
        contentVersion: catalog.contentVersion,
        figures: [...imageAssetsByModule.values()].reduce(
          (total, assets) => total + assets.size,
          0,
        ),
...
main().catch((error: unknown) => {
  ...
  console.error(`Curriculum catalog seed failed: ${message}`);
  process.exitCode = 1;
});
