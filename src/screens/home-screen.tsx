...
export function HomeScreen({
  ...
  const progress = overallLessons.length
    ? Math.round((completedCount / overallLessons.length) * 100)
    : 0;
  const examUnlocked =
    sections.length > 0 && sections.every((section) => completedSectionIds.has(section.id));
    ...
    >
      ...
      <Card style={styles.distanceCard}>
        <View style={styles.distanceHead}>
          ...
          <Text style={styles.distanceValue}>
            {completedSections.length} OF {sections.length} LEGS · {progress}%
          </Text>
        </View>
        <View style={styles.progressTicks}>
          {sections.map((section, index) => (
            <View
              ...
              style={[
                styles.progressTick,
                completedSectionIds.has(section.id) && styles.progressTickDone,
              ]}
            />
          ))}
        </View>
      ...
      <View style={styles.routeHeading}>
        ...
        <Text style={styles.routeCount}>
          {completedSections.length} OF {sections.length} LEGS
        </Text>
      </View>
      ...
      >
        <Card
          style={[styles.examCard, !examUnlocked && styles.lockedCard]}
          accent={examUnlocked ? colors.blue : colors.lineStrong}
        >
          <View
            style={[
              styles.examIcon,
              { backgroundColor: examUnlocked ? colors.bluePale : colors.paperDeep },
            ]}
          >
            <MaterialCommunityIcons
              name={examUnlocked ? 'shield-star-outline' : 'lock-outline'}
              size={25}
              color={examUnlocked ? colors.blue : colors.muted}
            />
          </View>
