...
describe("deterministic calculation generators", () => {
  it("generates the same complete practice set for the same seed", () => {
    expect(generateCalculationSet("student-42")).toEqual(
      generateCalculationSet("student-42"),
    );
    expect(generateCalculationSet("student-42")).not.toEqual(
      generateCalculationSet("student-43"),
    );
  });

  it("derives loaded CG from total moment and weight", () => {
    const problem = generateWeightAndBalance(7);
    expect(problem.centerOfGravity).toBeCloseTo(
      problem.totalMoment / problem.totalWeight,
      2,
    );
