...
export type CalculationCategory = 'weightAndBalance' | 'crosswind' | 'performanceInterpolation';
...
export type WeatherQuestionKind = 'wind' | 'visibility' | 'ceiling' | 'altimeter' | 'temperature';
...
export function generatePerformanceInterpolation(seed: Seed): PerformanceInterpolationProblem {
  ...
  const interpolatedDistance = round(
    linearInterpolate(targetAltitude, lowerAltitude, upperAltitude, lowerDistance, upperDistance),
  );
...
function formatSignedTemperature(value: number): string {
  return value < 0
    ? `M${String(Math.abs(value)).padStart(2, '0')}`
