...
export type QuestionResponse = ChoiceResponse | NumericResponse | MatchingResponse | null | undefined;
...
function parseNumberAndUnit(input: number | string):
...
function scoreMatching(question: MatchingQuestion, response: QuestionResponse): QuestionScore {
  ...
  return {
    ...
    score,
    maxScore: 1,
    reason: !answered ? "unanswered" : score === 1 ? "correct" : score > 0 ? "partiallyCorrect" : "incorrect",
  ...
  if (question.type === "numeric") {
    ...
    if (!parsed.valid || parsed.value === undefined) {
      return {
        ...
        score: 0,
        maxScore: 1,
        reason: response === null || response === undefined ? "unanswered" : parsed.reason ?? "invalidNumber",
        explanation: question.explanation,
